...
public class Gameplay1 extends JPanel implements KeyListener, ActionListener{
    ...
    private MapGenarator1 map1; 
    
    public Gameplay1(){
    map1 = new MapGenarator1(6, 10);
    addKeyListener(this);
    setFocusable(true);
    ...
    public void actionPerformed(ActionEvent e) {
       ...
       if(play){
           ...
           A: for(int i = 0; i<map1.map1.length; i++)
               for(int j = 0; j<map1.map1[0].length; j++){
                   if(map1.map1[i][j] > 0){
                       int brickX = j*map1.brickWidth1 + 80;
                       int brickY = i*map1.brickHeight1 + 50;
                       int brickWidth = map1.brickWidth1;
                       int brickHeigth = map1.brickHeight1;
